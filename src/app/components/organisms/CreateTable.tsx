...
function CreateColumn(props: any): React.JSX.Element {
  ...
  const column = useSelector(
    (store: RootState) =>
      (
        store.tabs.list[getTabIndexById(store.tabs.list, props.tabIndex)]
  ...
  return (
    <tr>
      ...
      <td>
        <select name="type" value={column.type} onChange={setColumnProperty}>
      ...
      <td>
        <i
          onClick={(e) => {
            e.preventDefault();
            dispatch(
              removeColumn({ tabId: props.tabIndex, column: props.slot })
            );
          }}
...
function CreateTable(props: any): React.JSX.Element {
  ...
  const currentNetwork = (useNetwork().chain as any).name;
  ...
  return (
    <form onSubmit={createTableOnNetwork}>
      ...
      <h2>
        Create Table on {currentNetwork}{" "}
        <i className="fa-solid fa-circle-question tooltip">
          <span>
            Switch networks to change the network this table will be created on.
          </span>
        </i>
      </h2>
      <label className="create-table-prefix">
        <div>Table Prefix</div>
        <input
      ...
      <div className="table-container">
        <table className="creating-table">
          ...
          <tbody>
            {(createColumns as any).map((column, key) => {
